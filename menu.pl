...
#kabai
  $form->{titlebar} = "Ledger ".$locale->text('Community')." ".$locale->text('Version'). "  - $myconfig{name} - $myconfig{dbname}";
...
sub check_password {
  ...
  if ($myconfig{password}) {
	...
	if ($form->{action} ne 'display') {
	  if ((! $cookie{"Ledger-$form->{login}"}) || $cookie{"Ledger-$form->{login}"} ne $form->{sessionid}) {
	    &getpassword(1);
	    exit;
	  }
	}
