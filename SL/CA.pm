...
sub all_transactions { #kabai 2.4.2
  my ($self, $myconfig, $form) = @_;
...
#kabai  
  if (!$form->{fx_transaction}) {
    $fxwhere = qq| AND ac.fx_transaction = '0'|;
  }
#kabai
...
#kabai BUG
      if ($department_id){
        $dpt_join2 = qq|JOIN dpt_trans d ON (d.trans_id = ac.trans_id)|;
        $dpt_where2 = qq|AND d.department_id = $department_id|;
      }
#kabai
