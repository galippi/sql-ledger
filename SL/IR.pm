...
#			      "onhand",
#			      qq|id = $form->{"id_$i"}|,
#kabai			      $form->{"qty_$i"}) unless $form->{shipped};
...
#kabai
	...
	$form->{fx}{$form->{fxloss_accno}}{$form->{"datepaid_$i"}} += $amount;
      ...
      if (($form->{currency} ne $form->{defaultcurrency}) && !$exchangerate) {
	$form->update_exchangerate_paid($dbh, $form->{currency}, $form->{"datepaid_$i"}, 0, $form->{"exchangerate_$i"});
      }
    }
  }
...
#kabai +1   
    # retrieve invoice
    $query = qq|SELECT a.invnumber, a.transdate, a.duedate, a.crdate,
                a.ordnumber, a.quonumber, a.paid, a.taxincluded, a.notes,
		a.intnotes, a.curr AS currency, a.vendor_id, a.language_code, a.scanned
		FROM ap a
		WHERE id = $form->{id}|;
...
sub get_vendor {
  ...
  my $query = qq|SELECT v.name AS vendor, v.creditlimit, v.terms,
		 ...
		 v.zipcode, v.country, v.curr AS currency, v.language_code,
                 $duedate AS duedate, v.notes AS vendornotes, v.intnotes AS vendorintnotes,
  ...
  if ($form->{id}) {
    map { delete $ref->{$_} } qw(currency taxincluded employee employee_id vendornotes vendorintnotes);
  }else{
    $form->{notes} = $ref->{vendornotes};
    $form->{intnotes} =  $ref->{vendorintnotes};
  }
...
#kabai 
  ...
  $query = qq|SELECT c.accno, c.description, c.link, t.rate, c.validfrom, c.validto
              FROM chart c
...
#KS
      if ($ref->{link} =~ /_paid/) {
	$j++;
	$form->{"AP_paid_$j"} = "$ref->{accno}";
      }

#kabai +1 BUG
      if ($ref->{category} eq 'L' && $ref->{link} !~ /_tax/) {
      	$form->{AP} = $form->{AP_1} = "$ref->{accno}";
...
#    $ref->{sellprice} = $form->round_amount($mref->{lastcost} * $form->{$mref->{curr}}, $decimalplaces);
    $ref->{sellprice} = $form->round_amount($mref->{lastcost}, $decimalplaces);
#kabai  
  }
  $pmh->finish;
  ...
  # connect to database, turn AutoCommit off
    my $dbh = $form->dbconnect_noauto($myconfig);
      ...
      foreach my $i (1 .. $form->{rowcount}) {
       ...
       $query = qq|SELECT transdate FROM invoice i, ap WHERE i.trans_id=ap.id AND ap.vendor_id IS NOT NULL AND i.parts_id=$form->{"id_$i"} AND ap.id <> $form->{id} ORDER BY transdate DESC LIMIT 1|;

       $sth2 = $dbh->prepare($query);
       $sth2->execute || $form->dberror($query);
       my $lastdate = $sth2->fetchrow_array;
       $sth2->finish;	

       if ($form->datetonum($lastdate, $myconfig) <= $form->datetonum($form->{transdate}, $myconfig)) {
	 $sth->execute($form->{"sellprice_$i"},$form->{"id_$i"}) || $form->dberror($query);
     	 $sth->finish;
       }
      }		     
