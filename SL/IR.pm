...
# JJR
  if ($myconfig->{dbdriver} eq 'DB2') {
    $duedate = ($form->{invdate}) ? "date('$form->{invdate}') + terms DAYS" : "current_date + terms DAYS";
  } else {
    $duedate = ($form->{invdate}) ? "to_date('$form->{invdate}', '$dateformat') + terms" : "current_date + terms";
  }
# End JJR
...
sub retrieve_item {
  ...
  my $query = qq|SELECT p.id, p.partnumber, p.description,
                 p.lastcost AS sellprice, p.unit,
