...
sub invoice_details {
  ...
  foreach $item (@sortlist) {
	...
	map { push(@{ $form->{$_} }, "") } qw(runningnumber number sku serialnumber bin qty unit deliverydate projectnumber sellprice listprice netprice discount discountrate linetotal);
      }
    }
	...
	while (my $ref = $sth->fetchrow_hashref(NAME_lc)) {
	  if ($form->{grouppartsgroup} && $ref->{partsgroup} ne $sameitem) {
	    map { push(@{ $form->{$_} }, "") } qw(runningnumber number sku serialnumber unit qty bin deliverydate projectnumber sellprice listprice netprice discount linetotal);
