...
sub save {
  ...
  if ($form->{id}) {
    ...
    # get old price
    $query = qq|SELECT sellprice, weight, lastcost
                FROM parts
		WHERE id = $form->{id}|;
    $sth = $dbh->prepare($query);
    $sth->execute || $form->dberror($query);

    my ($sellprice, $weight, $lastcost) = $sth->fetchrow_array;
    $sth->finish;
    ...
    while (my ($id, $qty) = $sth->fetchrow_array) {
      &update_assembly($dbh, $form, $id, $qty * 1, $sellprice * 1, $weight * 1, $lastcost * 1);
    }
    $sth->finish;
	...
	for $i (1 .. $form->{assembly_rows} - 1) {
          $sth->execute(($form->{"bom_$i"}) ? '1' : '0', $form->{id}, $form->{"id_$i"});
	  $sth->finish;
	}
      }
...
sub update_assembly {
  my ($dbh, $form, $id, $qty, $sellprice, $weight, $lastcost) = @_;
  ...
  while (my ($pid, $aqty) = $sth->fetchrow_array) {
    &update_assembly($dbh, $form, $pid, $aqty * $qty, $sellprice, $weight, $lastcost);
  }
  $sth->finish;
...
sub all_parts {
  ...
  if ($form->{make}) {
    ...
    $where .= " AND p.id IN (SELECT DISTINCT m.parts_id
  ...
  if ($form->{bought} || $form->{sold} || $form->{onorder} || $form->{ordered}) {
	...
	$invwhere .= " AND lower(i.description) LIKE '$var'";
      ...
      my $flds = qq|p.id, p.partnumber, i.description,
		    ...
		    p.priceupdate, p.image, p.drawing, p.microfiche,
		    pg.partsgroup,
		    a.invnumber, a.ordnumber, i.trans_id, a.till|;
