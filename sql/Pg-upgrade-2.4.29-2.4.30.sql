...
DROP FUNCTION g_accno_by_transid (integer, double precision,date);
...
if link LIKE ''%_amount%'' OR link SIMILAR TO ''%IC_(income|sale)%'' OR link SIMILAR TO ''(IC|%IC_expense%)'' then
 ...
 l := '''';
  ...
  query := ''select accno from chart where id in (SELECT chart_id from acc_trans where trans_id = ''''''||trans_id||'''''' ''||w||'' ''||l||'' AND transdate = ''''''||transdate||'''''' order by oid desc) '';
  w := '''';
...
if link LIKE ''%_amount%'' OR link SIMILAR TO ''%IC_(income|sale)%'' OR link SIMILAR TO ''(IC|%IC_expense%)'' then
 ...
 l := '''';
  ...
  query := ''select gifi_accno from chart where id in (SELECT chart_id from acc_trans where trans_id = ''''''||trans_id||'''''' ''||w||'' ''||l||'' AND transdate = ''''''||transdate||'''''' order by oid desc) '';
  w := '''';
  ...
  'LANGUAGE 'plpgsql';
-- end function
UPDATE defaults SET version = '2.4.30';
