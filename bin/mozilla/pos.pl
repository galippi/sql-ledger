...
sub form_header {
  ...
  foreach $item (qw(customer department)) {
...
<input type=hidden name=crdate value="|.$form->current_date(\%myconfig).qq|">
...
#kabai 
    if ($form->{"${item}_base"}) {
      ...
      $form->{"${item}_total"} = $form->format_amount(\%myconfig, $form->round_amount($form->{"${item}_total"},0), 2, 0);
...
|;
  ...
  $form->{invsubtotal} = $form->format_amount(\%myconfig, $form->round_amount($form->{invsubtotal},0), 2, 0);
...
|;
  ...
  $form->{change} = $form->format_amount(\%myconfig, $form->round_amount($form->{change}), 2, 0);
  $form->{totalpaid} = $form->format_amount(\%myconfig, $totalpaid, 2);

 
  $form->{oldinvtotal} = $form->{invtotal};
  $form->{invtotal} = $form->format_amount(\%myconfig, $form->round_amount($form->{invtotal},0), 2, 0);
...
|;
  ...
  if ($transdate > $closedto) {
    ...
    if ($form->{id} && $myconfig{role} =~ /(admin|manager)/) {
      print qq|
...
#kabai  
  $form->{invnumber}= $locale->text('RECEIPT');
#kabai
