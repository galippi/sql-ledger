...
# $locale->text('Services')
  ...
  $form->get_partsgroup(\%myconfig, 1);

  if (@{ $form->{all_partsgroup} }) {
    $partsgroup = qq|<option>\n|;
    
    map { $partsgroup .= "<option>$_->{partsgroup}\n" } @{ $form->{all_partsgroup} };
...
sub link_part {
  ...
  if ($form->{item} eq 'assembly') {
    ...
    if (@{ $form->{all_partsgroup} }) {
      ...
      map { $form->{selectassemblypartsgroup} .= "<option>$_->{partsgroup}\n" } @{ $form->{all_partsgroup} };
    }
  }
...
sub form_header {
  ...
  if ($form->{selectpartsgroup}) {
    $form->{selectpartsgroup} =~ s/ selected//;
    $form->{selectpartsgroup} =~ s/option>\Q$form->{partsgroup}\E/option selected>$form->{partsgroup}/;
...
sub assembly_row {
  ...
  @column_index = qw(runningnumber qty unit bom partnumber description total);
...
|;
  ...
  print qq|
  <input type=hidden name=selectassemblypartsgroup value="$form->{selectassemblypartsgroup}">
      </table>
    </td>
  </tr>
|;
...
sub update {
  ...
  if ($form->{item} eq 'service') {
    map { $form->{$_} = $form->parse_amount(\%myconfig, $form->{$_}) } qw(sellprice listprice lastcost);
...
# $locale->text('Assembly Number missing!')
  ...
  if ($form->{previous_form}) {
    ...
    if ($form->{item} eq 'assembly') {
      # undo formatting
      map { $form->{$_} = $form->parse_amount(\%myconfig, $form->{$_}) } qw(listprice sellprice lastcost weight);
      ...
      map { $form->{"${_}_$i"} = $newform{$_} } qw(partnumber description bin unit weight listprice sellprice lastcost inventory_accno income_accno expense_accno partsgroup);

      # undo string formatting
      map { $form->{"${_}_$i"} =~ s/''/'/g } qw(partnumber description unit bin);

      $form->{sellprice} += $form->{"sellprice_$i"} * $form->{"qty_$i"};
      $form->{sellprice} = $form->round_amount($form->{sellprice}, $dec1);
      
      $form->{weight} += $form->{"weight_$i"} * $form->{"qty_$i"};
      
      $form->{lastcost} += $form->{"lastcost_$i"} * $form->{"qty_$i"};
      $form->{lastcost} = $form->round_amount($form->{lastcost}, $dec1);

      $form->{listprice} += $form->{"listprice_$i"} * $form->{"qty_$i"};
      $form->{listprice} = $form->round_amount($form->{listprice}, $dec2);
