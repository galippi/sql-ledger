...
#kabai
  if ($showaccnumbers_true){
   map { $form->{selectaccount} .= "<option value=$_->{accno}>$_->{accno}--$_->{description}\n" } @{ $form->{PR}{"$form->{ARAP}_paid"} };
   map { $form->{"select$form->{ARAP}"} .= "<option value=$_->{accno}>$_->{accno}--$_->{description}\n" } @{ $form->{PR}{$form->{ARAP}} };
  }else{
   map { $form->{selectaccount} .= "<option value=$_->{accno}>$_->{description}\n" } @{ $form->{PR}{"$form->{ARAP}_paid"} };
   map { $form->{"select$form->{ARAP}"} .= "<option value=$_->{accno}>$_->{description}\n" } @{ $form->{PR}{$form->{ARAP}} };
  }  
#kabai
  # currencies
...
#  $form->{format} = "pdf" unless $myconfig{printer};
  if ($form->{regsource}) {
   $form->{regsource} =~ /(\D+)(\d+)/;
   my $nextreg = ($2*1) + 1;
   $form->{source} = "$1" . $nextreg;
   $form->{callback} .= "&regsource=$form->{source}"; 
  }
...
#kabai
  ...
  if ($form->{account}){
    $form->{selectaccount} =~ s/ selected//;
    $form->{selectaccount} =~ s/(<option value=\Q$form->{account}\E)/$1 selected/;
  }

  if ($form->{$form->{ARAP}}){

    ($form->{$form->{ARAP}}) = split /--/, $form->{$form->{ARAP}};
    $form->{"select$form->{ARAP}"} =~ s/ selected//;    
    $form->{"select$form->{ARAP}"} =~ s/(<option value=\Q$form->{$form->{ARAP}}\E)/$1 selected/;
  }
#kabai
#kabai +1  
  foreach $item (currency) {
    $form->{"select$item"} =~ s/ selected//;
...
#kabai
  my $inadvchecked = ($form->{inadvance}) ? "checked" : "";
  my $sourcetext = $form->{cash} ? "Cash voucher number" : "Source";
  my $required = $form->{cash} ? "" : "class=required";
...
<input type=hidden name=regsource value="$form->{regsource}">
  ...
  <tr>
    <td>
	  ...
	  <td align=right>
		...
		<td colspan=3><select name=$form->{ARAP} class="noscreen">$form->{"select$form->{ARAP}"}</select>
		</td>
		...
		<input type=hidden name=oldcurrency value=$form->{oldcurrency}>
	      ...
	      $exchangerate
	      <tr>
		<th align=right nowrap>|.$locale->text($sourcetext).qq|</th>
		<td colspan=3><input name=source $required value="$form->{source}" size=10></td>
	      </tr>
	      <tr>
		<th align=right nowrap>|.$locale->text('Amount').qq|</th>
		<td colspan=3><input name=amount class="required" size=10 value=|.$form->format_amount(\%myconfig, $form->{amount}, 2).qq|>
                &nbsp;|.$locale->text('In Advance').qq|<input name="inadvance" type=checkbox style=checkbox $inadvchecked>
                </td>
	      </tr>
	    </table>
	  </td>
...
#    map { $media .= qq| 
#          <option value="$_">$printer{$_}| } sort keys %printer;
#  }
#  if ($latex) {
#kabai NOT WORKING YET    $media .= qq|
#kabai          <option value=queue>|.$locale->text('Queue');
#    $format .= qq|
...
# $locale->text('Cannot post Receipt!')
 ...
 if (CP->process_payment(\%myconfig, \%$form)){
    if ($form->{regsource}) {
         use SL::AM;
	($toincrement) = $form->{regsource}=~ /^(\p{IsAlpha}+)\p{IsDigit}+$/;
...
sub print {
  ...
  @a = qw(name invnumber company address text_amount address1 address2 city state zipcode country memo);
...
#kabai BUG  $form->{datepaid} = $locale->date(\%myconfig, $form->{datepaid}, 0);
#kabai NOEMPTY_AMOUNT
  $form->error($locale->text('Amount is missing!')) if ($form->{amount} eq "");
#kabai
...
#kabai
    $form->error($locale->text('No invoice is checked!')) if !$form->{inadvance};
#kabai
