  ...
  $include = qq|
	...
	<tr>
	  ...
	  <td>
	    <table>
              <tr>
                <td><input name="l_subtotal" class=checkbox type=checkbox value=Y checked></td>
...
|;
  ...
  $include .= qq|
	      <tr>
		<td align=right>|.$locale->text('Shipping Address').qq|:</td>
		<td><input name="l_shiptoname" type=checkbox class=checkbox value=Y> |.$locale->text('Name').qq|</td>
		<td><input name="l_shiptocity" type=checkbox class=checkbox value=Y> |.$locale->text('City').qq|</td>
		<td><input name="l_shiptoaddress" type=checkbox class=checkbox value=Y> |.$locale->text('Address').qq|</td>
		<td><input name="l_shiptozipcode" type=checkbox class=checkbox value=Y> |.$locale->text('ZIP/Code').qq|</td>
	      </tr>
   | if ($form->{db} eq 'customer');
   $include .= qq|
  	      <tr>
...
sub list_names {
  ...
  @columns = $form->sort_columns(id, name, "$form->{db}number", address,
				 ...
				 shiptoname, shiptoaddress, shiptocity, shiptozipcode);
  ...
  $column_header{shiptoname} = qq|<th><a class=listheading href=$href&sort=shiptoname>|.$locale->text('Shipto Name').qq|</a></th>|;
  $column_header{shiptocity} = qq|<th><a class=listheading href=$href&sort=shiptocity>|.$locale->text('Shipto City').qq|</a></th>|;
  $column_header{shiptoaddress} = qq|<th class=listheading>|.$locale->text('Shipto Address').qq|</th>|;
  $column_header{shiptozipcode} = qq|<th><a class=listheading href=$href&sort=shiptozipcode>|.$locale->text('Shipto ZIP/Code').qq|</a></th>|;
...
|;
  ...
  foreach $ref (@{ $form->{CT} }) {
	...
	&list_subtotal;
      ...
      $column_data{shiptoaddress} = "<td>$ref->{shiptoaddress1} $ref->{shiptoaddress2}&nbsp;</td>";
...
# mEGYA. {
	$column_data{sellprice} = "<td align=right>".$form->format_amount(\%myconfig, $sellpricesubtotal, 2, "&nbsp;")."</td>";
	$column_data{fxsellprice} = "<td align=right>".$form->format_amount(\%myconfig, $fxsellpricesubtotal, 2, "&nbsp;")."</td>";

	$sellpricesubtotal = 0;
	$fxsellpricesubtotal = 0;
# mEGYA. }	
...
sub list_history {
  ...
  $href = "$form->{script}?action=list_history&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&type=$form->{type}&transdatefrom=$form->{transdatefrom}&transdateto=$form->{transdateto}&history=$form->{history}&l_subtotal=$form->{l_subtotal}";

  $form->sort_order();

  $callback = "$form->{script}?action=list_history&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&type=$form->{type}&transdatefrom=$form->{transdatefrom}&transdateto=$form->{transdateto}&history=$form->{history}&l_subtotal=$form->{l_subtotal}";
...
# mEGYA. {
      # print subtotal
      if ($form->{l_subtotal} && ($form->{history} eq 'summary') && $subtotal) {
	map { $column_data{$_} = "<td>&nbsp;</td>" } @column_index;
	&list_subtotal;
      }
# mEGYA. }
...
# mEGYA. {
    $sellpricesubtotal += $ref->{sellprice};
    $fxsellpricesubtotal += $ref->{fxsellprice};
# mEGYA. }
...
# mEGYA. {
  if ($form->{l_subtotal} && ($form->{history} eq 'summary')) {
    map { $column_data{$_} = "<td>&nbsp;</td>" } @column_index;
    &list_subtotal;
  }
# mEGYA. }
...
# $locale->text('Edit Vendor')
  ...
  # format discount
  $form->{discount} = $form->format_amount(\%myconfig, $form->{discount} *= 100, 2);
...
|;
  ...
  # currencies
  $form->{selectcurrency} = "<option value=''>";
  map { $form->{selectcurrency} .= "<option value='$_'\>$_\n" } split /:/, $form->{currencies};
  $form->{selectcurrency} =~ s/option value=('$form->{currency}')>/option value=$1 selected>/;
  
#kabai
  my $tax_text = $form->{db} eq "customer" ? "FIZ" : "VIS";  
  my $onclick;
#kabai
  foreach $item (split / /, $form->{taxaccounts}) {

    if ($form->{tax}{$item}{taxable}) {
     if ($form->{tax}{$item}{posted}){
      $onclick = qq|onclick="alert('|.$locale->text('Tax is already in use!').qq|');return false"|;
     }else{
      $onclick = "";
     } 
     $taxable .= qq| <input name="tax_$item" value=1 class=checkbox type=checkbox checked $onclick>&nbsp;<b>$form->{tax}{$item}{description}</b><br>| if $form->{tax}{$item}{taxnumber}=~ /$tax_text/;
    } else {
      if (!$form->{id}){
	$checked = $form->{tax}{$item}{base} ? "checked" : "";
      }
      $taxable .= qq| <input name="tax_$item" value=1 class=checkbox type=checkbox $checked>&nbsp;<b>$form->{tax}{$item}{description}</b><br>| if $form->{tax}{$item}{taxnumber}=~ /$tax_text/;
    }
  }
	...
	<tr>
	  ...
	  <td colspan=5>
	    <table>
	      <tr>
		<td>$taxable</td>
		<td><input name=taxincluded class=checkbox type=checkbox value=1 $form->{taxincluded}|;
		$tax.= qq| onclick= "return false"| if $form->{roti};
#		$tax.=qq|<input name=taxincluded class=checkbox type=hidden value=1 $form->{taxincluded}|;
		$tax.=  qq|></td>
...
|;
  ...
  if (@{ $form->{all_language} }) {
    ...
    $form->{selectlanguage} =~ s/(<option value="\Q$form->{language}--$form->{language_code}\E")/$1 selected/;
     my $langtext = "Language";
     my $hidden;
     if ($form->{db} eq "vendor"){
      $hidden = qq|class="noscreen"|;  
      $langtext = "";
      
     }
    $language = qq|
 	  <th align=right>|.$locale->text($langtext).qq|</th>
	  <td><select name=language $hidden>$form->{selectlanguage}</select></td>
|;
  }
...
|;
    ...
    $shipvia = qq|
	  <th align=right>|.$locale->text('Shipvia').qq|<label class="info super" title="|.$locale->text('for example: cash, bank transfer, cheque').qq|">?</label></th>
	  <td><input name=shipvia size=40 class="required" value="$form->{shipvia}"></td>
    |;
    $shippingpoint = qq|
	  <th align=right>|.$locale->text('Place').qq|</th>
	  <td><input name=shippingpoint size=40 value="$form->{shippingpoint}"></td>
    |;
    $required = qq|class="required"|;
  }  
...
|;
  ...
  if ($form->{db} eq "customer") {
   $terms = qq| 
    <table class="nowrap" border=1>
    <tr>
    <th>|.$locale->text('Terms').qq|<label class="info super" title="|.$locale->text('for example: 0 or 7').qq|">?</label></th>
    <td><input name=terms size=2 $required value="$form->{terms}"> <b>|.$locale->text('days').qq|</b> 
    </td>
    </tr>
    <tr>
    <th>|.$locale->text('Calculation method').qq|</th>
    <td>
    <INPUT type='radio' name='duebase' value='none' >| .$locale->text('None').qq| 
	   <INPUT type='radio' name='duebase' value='transdate' >|.$locale->text('Transdate').qq| 
	   <INPUT type='radio' name='duebase' value='crdate' >|.$locale->text('Crdate').qq| 
    </td>
    </tr>
    </table>
  |;
  ...
  $calc='transdate';
  $calc=$form->{duebase} if $form->{id} and $form->{duebase};
  $terms=~ s/\'$calc\'/\'$calc\' checked/;
...
<table width=100%>
    ...
    <td>
	  ...
	  <td width=50%>
		...
		<th class=listheading colspan=2 width=50%>|.$locale->text('Billing Address').qq|</th>
	      <tr>
		<th align=right nowrap>|.$locale->text($label .' Number').qq|</th>
		<td><input name="$form->{db}number" class="required" size=35 maxlength=32 value="$form->{"$form->{db}number"}"|
		 .qq|>&nbsp;U:&nbsp;|.$form->get_lastnum(\%myconfig, \%$form).qq|</td>
	      </tr>
	      <tr>
		<th align=right nowrap>|.$locale->text('Company Name').qq|</th>
		<td><input name=name class="required" size=35  id=name OnBlur="return quotchange('name');" maxlength=64 value="$form->{name}" |.$form->{ro}.qq|></td>
	      </tr>
	      <tr>
		<th align=right nowrap>|.$locale->text('ZIP/Code').qq|</th>
		<td><input name=zipcode $required size=10 maxlength=10 value="$form->{zipcode}" |.$form->{ro}.qq|></td>
	      </tr>
	      <tr>
	      <tr>
		<th align=right nowrap>|.$locale->text('City').qq|</th>
		<td><input name=city $required size=35 maxlength=32 value="$form->{city}" |.$form->{ro}.qq|></td>
	      </tr>
		<th align=right nowrap>|.$locale->text('Address').qq|</th>
		<td><input name=address1 $required size=35 maxlength=32 value="$form->{address1}" |.$form->{ro}.qq|></td>
	      </tr>
	      <tr>
		<th></th>
		...
		<td><input name=cc size=35 value="$form->{cc}"></td>
	      ...
	      $bcc
	        <tr>
		  <th align=right nowrap>|.$locale->text('Valid From').qq|
		  <td><input name=startdate size=11 title="yyyy-mm-dd" value="$form->{startdate}">
		  <B>&nbsp;&nbsp;&nbsp; |.$locale->text('Valid To').qq|</B>
		  <input name=enddate size=11 title="yyyy-mm-dd" value="$form->{enddate}"></td>
	        </tr>
	      </table>
	  </td>
	  <td width=50%>
  ...
  <tr>
	...
	<tr>
	  ...
	  <td><select class="required" Onchange="return setat();" name=currency>$form->{selectcurrency}</select></td>
	  $language
	</tr>
	<tr valign=top>
	  $employee
	...
	<tr valign=top>
	  ...
	  <td><textarea name=notes rows=3 cols=55 wrap=off>$form->{notes}</textarea></td>
	  <th align=right>|.$locale->text('Internal Notes').qq|</th>
	  <td><textarea name=intnotes rows=3 cols=35 wrap=off>$form->{intnotes}</textarea></td>
      </table>
    </td>
	</tr>
      </table>
...
sub form_footer {
  ...
  if ($form->{db} eq 'customer') {
    if ($myconfig{acs} !~ /AR--AR/) {
      $button{'AR--Add Transaction'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('AR Transaction').qq|"> |;
      $button{'AR--Add Transaction'}{order} = $i++;
      $button{'AR--Sales Invoice'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('Sales Invoice').qq|"> |;
      $button{'AR--Sales Invoice'}{order} = $i++;
    }
    if ($myconfig{acs} !~ /Order Entry--Order Entry/) {
      $button{'Order Entry--Sales Order'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('Sales Order').qq|"> |;
      $button{'Order Entry--Sales Order'}{order} = $i++;
    }
    if ($myconfig{acs} !~ /Quotations--Quotations/) {
      $button{'Quotations--Quotation'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('Quotation').qq|"> |;
      $button{'Quotations--Quotation'}{order} = $i++;
    }
  }
  if ($form->{db} eq 'vendor') {
    if ($myconfig{acs} !~ /AP--AP/) {
      $button{'AP--Add Transaction'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('AP Transaction').qq|"> |;
      $button{'AP--Add Transaction'}{order} = $i++;
      $button{'AP--Vendor Invoice'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('Vendor Invoice').qq|"> |;
      $button{'AP--Vendor Invoice'}{order} = $i++;
    }
    if ($myconfig{acs} !~ /Order Entry--Order Entry/) {
      $button{'Order Entry--Purchase Order'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('Purchase Order').qq|"> |;
      $button{'Order Entry--Purchase Order'}{order} = $i++;
    }
    if ($myconfig{acs} !~ /Quotations--Quotations/) {
      $button{'Quotations--RFQ'}{code} = qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text('RFQ').qq|"> |;
...
|;
  ...
  if ($form->{db} eq 'customer') {
#kabai +1
    $item = 'System--Customers--Add Customer';
  } 
  if ($form->{db} eq 'vendor') {
#kabai +1
    $item = 'System--Vendors--Add Vendor';
  } 
...
|;
    if ($form->{id}){
      print qq|<input class=submit type=submit name=action onclick="return checkform();" value="|.$locale->text("Save As New").qq|"> |;
    }
...
sub add_transaction {
  ...
  my $taxaccountno;
  map {  $taxaccountno += 1 if $_ =~ /tax_/} keys %$form;
  $form->error($locale->text('At least one tax account must be checked!')) if !$taxaccountno;  
  ...
  $form->{callback} = "$form->{script}?login=$form->{login}&path=$form->{path}&sessionid=$form->{sessionid}&action=add&vc=$form->{db}&$form->{db}_id=$form->{id}&$form->{db}=$name&type=$form->{type}&cash=$form->{cash}&callback=$form->{callback}";
...
sub save {
    $form->{name}=~ s/\"/\'/g; 
   map {$form->{$_} =~ s/\\/\//g} qw(name zipcode city address1 shiptoname shiptozipcode shiptocity shiptoaddress1); 
    $form->error($locale->text('This character is not allowed at the end of name: "-" !')) if $form->{name}=~ /.+-$/;
...
# d$locale->text('Customer deleted!')
