...
#kabai no href for postscript/PDF if invoice
    $column_data{spoolfile} = $form->{type} eq "invoice" ? qq|<td>$ref->{spoolfile}</td>| : qq|<td><a href=$spool/$ref->{spoolfile}>$ref->{spoolfile}</a></td>|;
