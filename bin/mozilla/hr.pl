	...
	<tr>
	  ...
	  <td colspan=3>
		...
		<td><input name="l_email" type=checkbox class=checkbox value=Y> |.$locale->text('E-mail').qq|</td>
	      ...
	      <tr>
		<td><input name="l_sin" type=checkbox class=checkbox value=Y> |.$locale->text('SSN').qq|</td>
...
sub list_employees {
  ...
  $href = "$form->{script}?action=list_employees&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&password=$form->{password}&status=$form->{status}";
  
  $form->sort_order();

  $callback = "$form->{script}?action=list_employees&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&password=$form->{password}&status=$form->{status}";
  ...
  $column_header{sin} = qq|<th><a class=listheading href=$href&sort=sin>|.$locale->text('SSN').qq|</a></th>|;
...
|;
  ...
  foreach $ref (@{ $form->{all_employee} }) {
    ...
    $column_data{name} = "<td><a href=$form->{script}?action=edit&db=employee&id=$ref->{id}&path=$form->{path}&login=$form->{login}&password=$form->{password}&status=$form->{status}&callback=$callback>$ref->{name}&nbsp;</td>";
...
sub search_deduction {
  ...
  $href = "$form->{script}?action=search_deduction&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&password=$form->{password}";

  $form->sort_order();

  $callback = "$form->{script}?action=search_deduction&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&password=$form->{password}";
...
|;
  ...
  foreach $ref (@{ $form->{all_deduction} }) {
    ...
    if ($ref->{description} ne $sameitem) {
      $column_data{description} = "<td><a href=$form->{script}?action=edit&db=$form->{db}&id=$ref->{id}&path=$form->{path}&login=$form->{login}&password=$form->{password}&callback=$callback>$ref->{description}</a></td>";
    } else {
